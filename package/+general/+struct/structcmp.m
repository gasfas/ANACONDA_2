...
function [Lout, message] = structcmp(S1, S2, varargin)
%
% Parsing inputs
%
p = inputParser;
...
if nS1~=nS2
        Lout = false;
        message = 'Structures have different length.';
        return
...
if nfnameS1~=nfnameS2
        Lout = false;
        message = 'Structures have different amount of fields.';
        return
...
for kk=1:nfnameS1
        ...
        if isstruct(RS1) && isstruct(RS2)
                ...
                Lvalue(kk) = general.struct.structcmp(RS1, RS2, 'Tab',ntab, 'IgnoreCase', p.Results.IgnoreCase, 'Report', p.Results.Report); % recursive calls in case of substructure
