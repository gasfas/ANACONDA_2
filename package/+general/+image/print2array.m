...
function [A, bcol] = print2array(fig, res, renderer, gs_options)
    ...
    if nargin > 2 && strcmp(renderer, '-painters')
        ...
        general.image.print2eps(tmp_eps, fig, 0, renderer, '-loose');
        try
            ...
            general.image.ghostscript(cmd_str);
        catch me
...
function fp = font_path()
    fp = general.image.user_string('gs_font_path');
    if ~isempty(fp)
        return
    end
    % Create the path
    ...
    general.image.user_string('gs_font_path', fp);
end
