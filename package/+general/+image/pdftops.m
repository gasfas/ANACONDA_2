...
path_ = general.image.user_string('pdftops');
...
if ~general.image.user_string('pdftops', path_)
    warning('Path to pdftops executable could not be saved. Enter it manually in %s.', fullfile(fileparts(which('general.image.user_string.m')), '.ignore', 'pdftops.txt'));
    return
end
end
