...
%}
    ...
    if epsLevel2
        ...
        if isPainters && ~general.image.using_hg2 && numel(findall(fig,'Type','image'))>1 && ~isempty(findall(fig,'Type','line'))
    ...
    if ~general.image.using_hg2(fig),  fig = double(fig);  end
    ...
    try
        ...
        fstrm = general.image.read_write_entire_textfile(name);
    catch
        fstrm = '';
    end
    ...
    try
        if ~isempty(fstrm) && general.image.using_hg2(fig)
            ...
            if ~isempty(hThinLines)
                ...
                fstrm_new = general.image.read_write_entire_textfile(name);
    ...
    if general.image.using_hg2(fig)
        ...
        fstrm = general.image.fix_lines(fstrm);
    end
    ...
    if bb_crop
        ...
        [A, bcol] = plot.fig.print2array(fig, 1, renderer);
        [aa, aa, aa, bb_rel] = plot.fig.crop_borders(A, bcol, bb_padding, crop_amounts);
    ...
    general.image.read_write_entire_textfile(name, fstrm);
end
