...
%
%   [hBar hErrorbar] = barwitherr(ax, ..) Plots the graphical objects into
%   the specified axes 'ax'
%
%   Symmetric Example:
...
function varargout = barwitherr(varargin)

% Plot into a certain axes, if specified:
if isgraphics(varargin{1}, 'Axes')
	ax = varargin{1};
	errors = varargin{2};
	try varargin = {varargin{3:end}};
	catch varargin = {}; end
else
	ax = gca;
	errors = varargin{1};
...
if nRows > 1
    ...
    hErrorbar = errorbar(ax, mean(x,1), values, lowerErrors, upperErrors, '.k');
