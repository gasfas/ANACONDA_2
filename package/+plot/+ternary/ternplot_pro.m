...
%%%%
Z = experimental(:, 3)';
[fA, fB, fC] = plot.ternary.fractions(experimental(:, 1)', experimental(:, 2)', experimental(:, 3)');
...
zg(Ag + Bg > 1) = nan;
% Make ternary axes
[hold_state, cax, next] = plot.ternary.ternaxes(num_axes_steps);
% plot data
...
[fA, fB, fC] = plot.ternary.fractions(dataA, dataB, dataC);
[x, y] = plot.ternary.terncoords(fA, fB, fC);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
plot.ternary.ternplot(dataA, dataB, dataC, 'majors', 10, '.', 'color', 'none')
set(gca, 'visible', 'off');
hold on
for i = 1: size(f2,1)
    if i < 2
