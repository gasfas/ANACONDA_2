...
function result = recurse(data, level, addit)
    if iscell(data) && ~IO.Yaml.ismymatrix(data)
...
function result = iter_struct(data, level, addit)
    ...
    for i = fields(result)'
        fld = char(i);
        if isstruct(result.(fld)) && isfield(result.(fld), IO.Yaml.kwd_parent())
            result.(fld) = inherit(result.(fld), result.(fld).(IO.Yaml.kwd_parent()), [], addit{1}, {});
        end;
    end;
end
