...
function result = ReadYaml(filename, nosuchfileaction, makeords, treatasdata, dictionary)
    ...
    ry = IO.Yaml.ReadYamlRaw(filename, 0, nosuchfileaction, treatasdata);

    ry = IO.Yaml.deflateimports(ry);
    if iscell(ry) && ...
        length(ry) == 1 && ...
        isstruct(ry{1}) && ...
    ...
    ry = IO.Yaml.mergeimports(ry);    
    ry = IO.Yaml.doinheritance(ry);
    ry = IO.Yaml.makematrices(ry, makeords);    
    if exist('dictionary','var')
        ry = IO.Yaml.dosubstitution(ry, dictionary);
    end
