...
% Histogram metadata:
s.e_X.hist.binsize		= 0.08;% [mm] binsize of the variable. 
...
% Histogram metadata:
s.e_Y.hist.binsize		= 0.08;% [mm] binsize of the variable. 
...
% Histogram metadata:
s.e_E.hist.binsize		= 0.1;% [eV] binsize of the variable. 
s.e_E.hist.Range			= [-1 10];% [eV] range of the variable. 
% Axes metadata:
s.e_E.axes.Lim			= [-1 10];% [eV] Lim of the axis that shows the variable. 
s.e_E.axes.Tick			=  linspace(-1, 11, 2);% Ticks shown 
...
try 
	...
	s.i_TOF.axes.Tick	= s.i_TOF.hist.Range(1):100:s.i_TOF.hist.Range(2);
end
...
try 
	...
	s.i_TOF_bgr.axes.Tick = s.i_TOF_bgr.hist.Range(1):100:s.i_TOF_bgr.hist.Range(2);
end
...
try 
	...
	s.i_m2q_l.axes.Tick	= s.i_m2q_l.hist.Range(1):10:s.i_m2q_l.hist.Range(2);
end
...
try 
	...
	s.i_m2q_l_sum.axes.Tick	= s.i_m2q_l_sum.hist.Range(1):10:s.i_m2q_l_sum.hist.Range(2);
end
