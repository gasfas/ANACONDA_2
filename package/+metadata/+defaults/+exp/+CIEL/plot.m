...
d1.theta_R.figure.Position		= [100 100 440 460];
d1.theta_R.axes.YLim			= [20 35];
d1.theta_R.axes(1).Title.String			= 'Theta vs R';
...
d1.dpxy.figure.Position		= [300 300 440 460];
% d1.dpxy.figure.Position		= plot.fig.Position('NE');
d1.dpxy.axes.axis				= 'equal';
d1.dpxy.axes.Title.String		= 'Electron momentum';
...
d2.TOF							= metadata.create.plot.signal_2_plot({signals.TOF});
d2.TOF.hist.Integrated_value	= 1;
d2.TOF.GraphObj.ax_nr			= 1;
d2.TOF.figure.Position			= [200 200 500 500];
d2.TOF.axes.Position			= [0.05 0.2 0.9 0.6];
d2.TOF.axes(1).Title.String		= 'Ion TOF';
% Axes properties:
d2.TOF.axes(1).YTick			= linspace(0, 0.01, 10);
d2.TOF.axes(1).YLim				= [0 0.01];
% d2.TOF.axes						= macro.plot.add_axes(d2.TOF.axes(1), signals.add_m2q.axes, exp_md.conv.det2, 'm2q', 'X');

d2.TOF_hit1_hit2				= metadata.create.plot.signal_2_plot({signals.TOF, signals.TOF});
...
d2.TOF_X						= metadata.create.plot.signal_2_plot({signals.TOF, signals.i_X});
d2.TOF_X.figure.Position		= [200 200 440 460];
% d2.TOF_X.axes.Title.String        = 'TOF vs X position';
...
d2.TOF_R						= metadata.create.plot.signal_2_plot({signals.TOF, signals.i_R});
d2.TOF_R.figure.Position		= [200 200 440 460];
