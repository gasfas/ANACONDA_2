  ...
  elseif isstruct(array)
    ...
    if concatenate_horizontally % Extract one row at a time
      for i = 1:size(array, 1)
        if inner_struct
          y(i,:) = struct(IO.DLT2ANA.collect_field(array(i,:), fieldname, true));
        else
          y(i,:) = IO.DLT2ANA.collect_field(array(i,:), fieldname, true);
        end
      end
    else % Extract one column at a time (This is the default)
      for j = 1:size(array, 2)
        if inner_struct
          y(:,j) = struct(IO.DLT2ANA.collect_field(array(:,j), fieldname));
        else
          y(:,j) = IO.DLT2ANA.collect_field(array(:,j), fieldname);
        end
      end
    end
    return;
