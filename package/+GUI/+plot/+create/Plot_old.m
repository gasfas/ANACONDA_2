...
if plotsettings(4) == 1 % Plot all in new figure.
    if numberofloadedfilesselected == 1 %One file to plot - only one exp
        ...
        GUI.plot.create.plot_types.plot_2(data_n.([char(exp_names)]), mdata_n.([char(exp_names)]), graphtype_X, exp_names, dimensions, md_GUI);
    elseif numberofloadedfilesselected > 1  %Multiple files to plot - multiple exp
        ...
        GUI.plot.create.plot_types.plot_2_n(data_n, mdata_n, exp_name, graphtype_X, dimensions, md_GUI);
    end
elseif plotsettings(4) == 2 % Plot all separately.
    if numberofloadedfilesselected == 1 %One file to plot - only one exp
        ...
        GUI.plot.create.plot_types.plot_2(data_n.([char(exp_name)]), mdata_n.([char(exp_name)]), graphtype_X, exp_name, dimensions, md_GUI);
    elseif numberofloadedfilesselected > 1  %Multiple files to plot - multiple exp
        for l = 1:numberofloadedfilesselected
            detectorname = char(detectors.([char(exp_names(l))]));
            number = l;
...
elseif plotsettings(4) == 3 % Plot selection into pre-existing figure.
    ...
    %ax = GUI.plot.create.plot_types.plot_4(p_pyridine_p_d.(exp_name), p_pyridine_p_md.(exp_name), graphtype_X_selected_number, figureselected, experiment_name);
    %% Fix GUI.plot.create.plot_types.plot_3 and GUI.plot.create.plot_types.plot_3_n. Look at macro_plot_4. Problem with expnumber if experiment_selected_number(l) >= 10...
    if numberofloadedfilesselected == 1 %One file to plot - only one exp
        ...
        ax = GUI.plot.create.plot_types.plot_3(data_n.(exp_name), mdata_n.(exp_name), graphtype_X, figureselected, experiment_name, dimensions, md_GUI);
    elseif numberofloadedfilesselected > 1  %Multiple files to plot - multiple exp
        detectorname = char(detectors.([char(exp_names(l))]));
        for l = 1:numberofloadedfilesselected
            expnumber(l) = int2str(experiment_selected_number(l))
        ...
        ax = GUI.plot.create.plot_types.plot_3_n(data_n, mdata_n, exp_names, graphtype_X, figureselected, experiment_name, dimensions, md_GUI);
    end
end
end