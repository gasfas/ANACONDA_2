function h_f = Plot_scans(exps, mds)
% Function to plot scans of different samples that can be manipulated in a
...
UI_obj.plot.m2q.main = uifigure('Name', ['Channel yield spectra, exp names: ' + join(string(sample_names), ', ')], ...
    'NumberTitle','off', 'position', [200 200 300 300], 'KeyPressFcn', @keypress_callback);
UI_obj.plot.m2q.plot_window = figure('Name', 'Plot channel yield spectra', ...
...
UI_obj.plot.m2q.holdchbx = uidropdown(UI_obj.plot.m2q.main, 'Items', sample_names, ...
    'ValueChangedFcn', @change_sample_dropdown_callback, 'Position', [10, 150, 120, 25]);
why
...
function[hLine] = update_plot(ax, exps, sample_name, spectrum_number)
    ...
    for sample_nr = 1:length(sample_names)
        sample_name = sample_names{sample_nr};
        spectrum_names = fieldnames(exps.(sample_name).hist);
        BR          = zeros(length(spectrum_names),1);
        for spectr_nr_cur = 1:length(spectrum_names)
            BR(spectr_nr_cur,1) = sum(exps.(sample_name).hist.(spectrum_names{spectr_nr_cur}).M2Q.I(1:end));
        end
        LineName_leg    = [sample_name];
        hLine           = plot(UI_obj.plot.m2q.axes, exps.(sample_name).photon.energy, BR, 'DisplayName', LineName_leg);
        hold on
    end
    ...
    xlabel(UI_obj.plot.m2q.axes, 'Photon energy [eV]')
