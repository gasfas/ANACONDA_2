...
function load_scan_Keypress_callback(hObj, event)
...
function remove_scan_GUI(~, ~)
    ...
    elseif isempty(UI_obj.main.scan.uitable.Selection)
        ...
        for sample_nr       = 1:length(rownrs)
            ...
            exp_data.scans        = rmfield(exp_data.scans, settings.filelist.scan_name{sample_selected});
        end
        % Update the table:
        uitable_scan_modify();
    end
...
function load_scan_OK_callback(~, ~)
    ...
    if isfield(exp_data.scans, UI_obj.load_scan.sample_name.Value) || ~isvarname(UI_obj.load_scan.sample_name.Value)
        ...
        try set(UI_obj.load_scan.loading_data_msgbox, 'visible', 'off'); end
...
function load_scan_cancel_callback(~,~)
    ...
    function uitable_scan_user_edit(hObj, event)
        ...
        if event.Indices(2) == 1 % Name change
            ...
            if ~isempty(NewName) & ~any(ismember(fieldnames(exp_data.scans), NewName))
                ...
                exp_data.scans.(NewName)      = exp_data.scans.(scan_names{event.Indices(1)});
                settings.metadata.(NewName)   = settings.metadata.(scan_names{event.Indices(1)});
                settings.filelist.scan_name{event.Indices(1)}  = NewName;
                % remove old name:
    ...
    function uitable_scan_user_select(hObj, event)
        % If the user wants to change the color of the scan (relevant in later plots):
         % Callback to update the requested plot line color.
        if ~isempty(hObj.Selection) & all(unique(hObj.Selection(:,2)) == 5) % Only column 2 selected.
            switch unique(hObj.Selection(2))
                case 5 % The user wants to change the color of the line. TODO.
                        % Get the current color:
                        scan_names      = fieldnames(exp_data.scans);
                        name_active_scan = scan_names{hObj.Selection(1,1)};
                        current_color_RGB_cell = settings.filelist.color(hObj.Selection(1,1));
                        ...
                        for  i = 1:size(hObj.Selection, 1)% Possibly more than one scan needs to be re-colored:
                            ...
                            UI_obj.main.scan.uitable.Data{hObj.Selection(i,1), hObj.Selection(i,2)} = regexprep(num2str(round(newColorRGB,1)),'\s+',',');
                            settings.filelist.color{hObj.Selection(i,1)}        = newColorRGB;
                            settings.metadata.(name_active_scan).Color          = newColorRGB;
                            exp_data.scans.(name_active_scan).Color             = newColorRGB;
                        end

                        % Change the color of the cell to the newly selected one:
                        s = uistyle('BackgroundColor', newColorRGB);
                        addStyle(UI_obj.main.scan.uitable, s, 'cell', hObj.Selection);
            end
        end
    end
    ...
    function uitable_scan_modify()
        ...
        settings.filelist.color                 = cell(length(settings.filelist.scan_name),1);
        ...
        for name_nr = 1:length(settings.filelist.scan_name)
            ...
            try settings.filelist.number_of_spectra{name_nr}           = length(fieldnames(exp_data.scans.(name_cur).hist));
             ...
             settings.filelist.color{name_nr}                 = regexprep(num2str(round(settings.metadata.(name_cur).Color ,1)),'\s+',',');
             % Change the background color of the color cells:
            s = uistyle('BackgroundColor', settings.metadata.(name_cur).Color);
            addStyle(UI_obj.main.scan.uitable, s, 'cell', [name_nr,5]);

        end
        % Fill the metadata into the table:
        UI_obj.main.scan.uitable.Data             = [settings.filelist.scan_name, settings.filelist.number_of_spectra settings.filelist.photon_energy_min, settings.filelist.photon_energy_max, settings.filelist.color];
