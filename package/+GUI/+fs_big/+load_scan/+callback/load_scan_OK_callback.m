...
if ismember(UI_obj.load_scan.sample_name.Value, user_scannames) || ismember(UI_obj.load_scan.sample_name.Value, user_spectranames)
    ...
    scan_name_cur    = ['scan_' , num2str(GUI_settings.scans.scan_nr_cur, '%03.f')];
    spectr_name_cur = ['spectrum_' , num2str(GUI_settings.spectra.spectrum_nr_cur, '%03.f')];
        ...
        case 'Spektrolatius'
            switch UI_obj.load_scan.isscanfields.rb_individual_spectra.Value
                case true % Individual spectra (not a scan) loaded:
                    ...
                    if ischar(GUI_settings.load_scan.csv_filelist) % If only one file user-selected:
                        exp_data.spectra.(spectr_name_cur).Data = IO.SPECTROLATIUS_S2S.load_CSV_filelist(GUI_settings.load_scan.csv_filedir, GUI_settings.load_scan.csv_filelist , GUI_settings.load_scan.re_bin_factor);
                        exp_data.spectra.(spectr_name_cur).Name                     = UI_obj.load_scan.sample_name.Value;                         
                        exp_data.spectra.(spectr_name_cur).Color                    = plot.colormkr(GUI_settings.filelist.color_counter,1);
                        GUI_settings.spectra.spectrum_nr_cur                        = GUI_settings.spectra.spectrum_nr_cur + 1;
                        GUI_settings.filelist.color_counter                         = GUI_settings.filelist.color_counter + 1;
                        ...
                        for spectr_nr = 1:length(GUI_settings.load_scan.csv_filelist)
                            try
                                exp_data.spectra.(spectr_name_cur)                      = IO.SPECTROLATIUS_S2S.load_CSV_filelist(GUI_settings.load_scan.csv_filedir, GUI_settings.load_scan.csv_filelist{spectr_nr} , GUI_settings.load_scan.re_bin_factor);
                                ...
                                file_found = false;
                                UI_obj.load_scan.file_not_found = msgbox(['File ', GUI_settings.load_scan.csv_filelist{spectr_nr}, ' not found. Make sure the filename is correct.']);
                            end
                        end
                    end
                case false % A scan is loaded, so save the files in exp_data.scans:
                    ...
                    GUI_settings.scans.scan_nr_cur = GUI_settings.scans.scan_nr_cur + 1;      
                   ...
                   if is_modify_scan
                       ...
                       exp_data.scans.(scan_name_cur).Color                = plot.colormkr(GUI_settings.filelist.color_counter,1);
                   end
                   GUI_settings.filelist.color_counter                              = GUI_settings.filelist.color_counter + 1;
