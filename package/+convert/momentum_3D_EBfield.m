function [p, p_0] = momentum_3D_EBfield(TOF, X, Y, m2q_l, m_l, labels, labels_mass, labels_TOF_no_p, E_ER, Bfield, sample_md) 
...
% p_X_0 = 0.5*general.constants({'q'}).*Bfield.*((sin(omega.*T_0.*1e-9)./(1-cos(omega.*T_0.*1e-9))).*X_0 - (sin(omega.*TOF_no_dp.*1e-9)./(1-cos(omega.*TOF_no_dp.*1e-9))).*X_no_p - (Y_0 - Y_no_p))*1e-3;%[kg*m/s]
% p_Y_0 = 0.5*general.constants({'q'}).*Bfield.*(X_0 - X_no_p -(sin(omega.*T_0.*1e-9)./(1-cos(omega.*T_0.*1e-9))) .*Y_0 - (sin(omega.*TOF_no_dp.*1e-9)./(1-cos(omega.*TOF_no_dp.*1e-9))).*Y_no_p)*1e-3;%[kg*m/s]
% 
% p_Z_0 = ch_l_f.*general.constants({'q'}).* E_ER.* (T_0 - TOF_no_dp)*1e-9; % [kg*m/s] [m,1]
