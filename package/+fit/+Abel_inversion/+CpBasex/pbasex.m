...
if ischar(gData)
    gData = fit.Abel_inversion.CpBasex.loadG(gData);
end
...
if makeImages
    im_recon = fit.Abel_inversion.CpBasex.unfoldQuadrant(reshape(gData.Up'*(diag(gData.S)*(gData.V'*c)),nx,nx,nims));
    im_inv = fit.Abel_inversion.CpBasex.unfoldQuadrant(reshape(gData.Ginv*c,nx,nx,nims));
end
