function [ DF, FP ] = edge(xdata, ydata, nof_peaks)
...
DF.y_bgr.value  = mean(ydata);
DF.y_bgr.isfree = true; % only the first one counts.
DF.y_bgr.min    = 0;
DF.y_bgr.max    = max(ydata);

DF.edge.value   = mean([min(xdata), max(xdata)]);
DF.edge.isfree  = true;
DF.edge.min     = min(xdata);
DF.edge.max     = max(xdata);
DF.edge.function= 'linear';

DF.edge_slope.value     = peak2peak(ydata)./peak2peak(xdata)*2;
DF.edge_slope.isfree    = true;
DF.edge_slope.min       = 0;
DF.edge_slope.max       = DF.edge_slope.value*4;
DF.edge_slope.function  = 'linear';
