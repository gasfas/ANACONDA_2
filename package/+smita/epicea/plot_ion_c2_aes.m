function [Tet_ev_mean,Tet_ev_std,Tet_ev_mode] = plot_ion_c2_aes(data_converted, data_stats,e_filter_roi,plot_signal,m2q_1,m2q_2)
...
e_TRG.e.data_pointer	= 'h.det1.KER';
e_TRG.e.translate_condition = 'AND';
e_TRG.e.value		= data_stats.e_KER_range;
...
%     errorbar(Bincenters,Tet_ev,Tet_ev_error, '-k','LineWidth',2, 'DisplayName','Total KER')
    errorbar(Bincenters,Tet_ev./max(Tet_ev),Tet_ev_error./max(Tet_ev), '-k','LineWidth',2, 'DisplayName','Total KER')
    xline(Tet_ev_mean)
% %     xline(Tet_ev_mode)
    yl= ylim;
