function  [data_out] = group_hist(data_in, metadata_in, det_name)
...
for i = 1:length(detnames);
    ...
    q               = metadata_in.conv.(detname).q_label_hist.q;
    search_radius   = metadata_in.conv.(detname).q_label_hist.search_radius;
    M_n             = metadata_in.conv.(detname).q_label_hist.n.mass;
    M_m             = metadata_in.conv.(detname).q_label_hist.m.mass;
    M_H             = metadata_in.conv.(detname).q_label_hist.H.mass;
    nof_H           = metadata_in.conv.(detname).q_label_hist.H.nof;% number of protons
    ...
    if isfield(metadata_in.conv.(detname).q_label_hist, 'bgr_subtr')
        bgr_factor                              = metadata_in.conv.(detname).q_label_hist.bgr_subtr.factor;
        if isfield(metadata_in.conv.(detname).q_label_hist.bgr_subtr, 'filename')
            % Load the background data:
            bgr =   IO.import_raw(metadata_in.conv.(detname).q_label_hist.bgr_subtr.filename);
            bgr_md= IO.import_metadata(metadata_in.conv.(detname).q_label_hist.bgr_subtr.filename);
            bgr = macro.correct(bgr, bgr_md);
            bgr = macro.convert(bgr, bgr_md);
            method          = 'Hits';
            bgr_1 =  bgr.h.(detname).m2q;
            bgr_2 = bgr_factor;
        elseif strcmpi(metadata_in.conv.(detname).q_label_hist.bgr_subtr.method, 'flat_noise') 
            method          = 'Intensity';
