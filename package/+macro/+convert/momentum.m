...
% 				labels_TOF_no_p = convert.m2q_2_TOF(labels, m2qfactor, t0);
			...
			case 'electron' % if the detector sees electrons:
                ...
                labels          = metadata_in.conv.(detname).m2q_labels;
                labels_mass     = labels;
				labels_TOF_no_p = calc_labels_TOF_no_p(labels_mass/labels, metadata_in.conv.(detname).TOF_2_m2q); %[ns] important to notice here is that the mass and charge are both taken
                ...
                if general.struct.probe_field(metadata_in.spec, 'is_B_field')                    
                    [data_out.h.(detname).p, data_out.h.(detname).p_0] = momentum_2D_EBfield(TOF, X, Y, m2q_l, m_l, labels, labels_mass, labels_TOF_no_p, E_ER, metadata_in.spec.B_field, sample_md);    
                    data_out.h.(detname).dp = data_out.h.(detname).p - data_out.h.(detname).p_0;
