...
% row. 
%   x_range is a column array with the values at which the 
%   user wants to see the counts.
%   Count is a 2D-array with the elements giving the number of points from
...
% We loop over the rows, one by one:
% for i = 1:size(patterns, 1)
% 	Count(i) = sum(all(x_data == patterns(i,:), 2));
% end