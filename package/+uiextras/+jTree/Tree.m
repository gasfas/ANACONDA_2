    ...
    methods (Access = protected)
        ...
        function onNodeDND(tObj,e)
            ...
            if callbacksEnabled(tObj)
                ...
                % Get the target node
                Loc = e.getLocation();
                treePath = tObj.jTree.getPathForLocation(Loc.getX, Loc.getY); % This was changed by Benjamin Bolling; Optimized for MATLAB R2016a, macOS SIERRA.
                % treePath = tObj.jTree.getPathForLocation(Loc.getX + tObj.jScrollPane.getHorizontalScrollBar().getValue(), Loc.getY + tObj.jScrollPane.getVerticalScrollBar().getValue()); %before the change
                if isempty(treePath)
                ...
                % Used to show the position of the target node during DnD:
                %disp(TargetNode.Name) % Added by Benjamin Bolling.
